...
class Base44Service
{
    ...
    public function listEntities(string $entity, array $filters = [], array $sort = [], int $limit = 100): array
    {
        ...
        return $this->request('GET', "{$this->appId}/entities/{$entity}", $query);
    }

    public function getEntity(string $entity, string $id): array
    {
        return $this->request('GET', "{$this->appId}/entities/{$entity}/{$id}");
    }

    public function createEntity(string $entity, array $data): array
    {
        return $this->request('POST', "{$this->appId}/entities/{$entity}", $data);
    }

    public function updateEntity(string $entity, string $id, array $data): array
    {
        return $this->request('PATCH', "{$this->appId}/entities/{$entity}/{$id}", $data);
    }

    public function uploadFile(string $filePath, string $mimeType, string $fileName): ?string
    {
        try {
            $response = $this->client->request('POST', "{$this->appId}/files/upload", [
                'multipart' => [
                    [
                        'name'     => 'file',
    ...
    {
        try {
            return $this->request('POST', "{$this->appId}/functions/getAgentConfig");
        } catch (\Exception $e) {
            return null;
        }
    }
    ...
    {
        return $this->request('POST', "{$this->appId}/functions/updateAgentConfig", $data);
    }
